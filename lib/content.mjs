export function validateContent(d){
 ...
 if(!/^#[0-9a-f]{6}$/i.test(d.brand.red))fail('Color inválido');asset(d.brand.logo,'logo');asset(d.brand.heroImage,'foto principal');
 ...
 array(d.plans,'planes',10);d.plans.forEach(p=>{text(p.name,'plan',100);text(p.tagline,'subtítulo',300);if(!Number.isFinite(p.price)||p.price<0||p.price>1000000)fail('Precio inválido');if(typeof p.featured!=='boolean')fail('Plan destacado inválido');strings(p.items,'detalle del plan');});
 ...
 text(d.notes?.metrics,'nota de métricas');text(d.notes?.pricing,'nota de precios');return d;
}
